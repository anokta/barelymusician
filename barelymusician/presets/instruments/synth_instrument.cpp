...
void SynthInstrument::NoteOff(double pitch) noexcept { voice_.Stop(pitch); }

void SynthInstrument::NoteOn(double pitch, double intensity) noexcept {
...
BarelyInstrumentDefinition SynthInstrument::GetDefinition() noexcept {
  static std::vector<BarelyParameterDefinition> parameter_definitions = {
      ...
      // Decay.
      BarelyParameterDefinition{0.0, 0.0, 60.0},
      // Sustain.
...
BarelyStatus BarelyExamples_CreateSynthInstrument(BarelyApi api,
  ...
  return BarelyInstrument_Create(api,
                                 barelyapi::SynthInstrument::GetDefinition(),
