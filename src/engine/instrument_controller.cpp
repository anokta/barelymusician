...
BarelyRef InstrumentController::Acquire(const BarelyInstrumentControlOverride* control_overrides,
                                        int32_t control_override_count) noexcept {
  const uint32_t instrument_index = instrument_pool_.Acquire();
  if (instrument_index == 0) {
    return {};
  }

  InstrumentState& instrument = instrument_pool_.Get(instrument_index);
  instrument = {};
  instrument.controls = BuildControlArray(control_overrides, control_override_count);

  message_queue_.Add(update_frame_, InstrumentCreateMessage{instrument_index});
  for (int i = 0; i < BarelyInstrumentControlType_kCount; ++i) {
    message_queue_.Add(
        update_frame_,
        InstrumentControlMessage{instrument_index, static_cast<BarelyInstrumentControlType>(i),
                                 instrument.controls[i].value});
  }

  return {instrument_index, instrument_pool_.GetGeneration(instrument_index)};
}
...
void InstrumentController::SetAllNotesOff() noexcept {
  for (uint32_t i = 0; i < instrument_pool_.GetActiveCount(); ++i) {
    SetAllNotesOff(instrument_pool_.GetActiveIndex(i));
  }
}
...
void InstrumentController::SetAllNotesOff(uint32_t instrument_index) noexcept {
  ...
  if (instrument.arp.is_active) {
    ...
    message_queue_.Add(update_frame_, NoteOffMessage{instrument_index, instrument.arp.pitch});
  } else {
    for (const float pitch : std::exchange(instrument.pitches, {})) {
      instrument.note_event_callback(BarelyNoteEventType_kEnd, pitch);
      message_queue_.Add(update_frame_, NoteOffMessage{instrument_index, pitch});
    }
  }
}
  ...
  if (auto& control = instrument.controls[type]; control.SetValue(value)) {
    switch (type) {
        ...
        if (static_cast<BarelyArpMode>(value) == BarelyArpMode_kNone) {
          if (instrument.arp.pitch_index != -1) {
            ...
            message_queue_.Add(update_frame_,
                               NoteOffMessage{instrument_index, instrument.arp.pitch});
            for (const auto& [pitch, note_controls] : instrument.note_controls) {
              ...
              message_queue_.Add(update_frame_, NoteOnMessage{instrument_index, pitch,
                                                              BuildNoteControls(note_controls)});
            }
          }
      ...
      case BarelyInstrumentControlType_kArpRate:
        break;
      default:
        message_queue_.Add(update_frame_, InstrumentControlMessage{instrument_index, type, value});
        break;
    }
  }
}
...
void InstrumentController::SetNoteEventCallback(uint32_t instrument_index,
  ...
  instrument_pool_.Get(instrument_index).note_event_callback = {callback, user_data};
}
...
float InstrumentController::GetControl(uint32_t instrument_index,
  ...
  return instrument_pool_.Get(instrument_index).controls[type].value;
}
...
const float* InstrumentController::GetNoteControl(uint32_t instrument_index, float pitch,
  ...
  const auto& instrument = instrument_pool_.Get(instrument_index);
  if (const auto it = instrument.note_controls.find(pitch); it != instrument.note_controls.end()) {
...
bool InstrumentController::IsNoteOn(uint32_t instrument_index, float pitch) const noexcept {
  return instrument_pool_.Get(instrument_index).IsNoteOn(pitch);
}
    ...
    if (!instrument.arp.is_active && instrument.arp.phase == 0.0) {
      ...
      switch (static_cast<BarelyArpMode>(
        ...
        case BarelyArpMode_kRandom:
          instrument.arp.pitch_index = main_rng.Generate(0, size);
          break;
        default:
      ...
      message_queue_.Add(
          update_frame_,
          NoteOnMessage{instrument_pool_.GetActiveIndex(i), instrument.arp.pitch,
                        BuildNoteControls(instrument.note_controls.at(instrument.arp.pitch))});
