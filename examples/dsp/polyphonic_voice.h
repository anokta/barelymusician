#ifndef EXAMPLES_DSP_POLYPHONIC_VOICE_H_
#define EXAMPLES_DSP_POLYPHONIC_VOICE_H_
...
void PolyphonicVoice<VoiceType>::Start(
    float pitch, const VoiceCallback& init_voice) noexcept {
...
void PolyphonicVoice<VoiceType>::Stop(
    float pitch, const VoiceCallback& shutdown_voice) noexcept {
  const int num_voices = static_cast<int>(voices_.size());
  for (int i = 0; i < num_voices; ++i) {
...
#endif  // EXAMPLES_DSP_POLYPHONIC_VOICE_H_
