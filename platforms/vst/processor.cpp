...
Steinberg::tresult PLUGIN_API Processor::process(Steinberg::Vst::ProcessData& data) {
  ...
  // Process instrument.
  engine_->Process(output_samples_, /*timestamp=*/0.0);
  for (int frame = 0; frame < static_cast<int>(data.numSamples); ++frame) {
...
Steinberg::tresult PLUGIN_API Processor::setupProcessing(Steinberg::Vst::ProcessSetup& setup) {
  ...
  instrument_ = std::nullopt;  // destroy existing instrument first
  engine_ = Engine(static_cast<int>(setup.sampleRate), static_cast<int>(output_samples_.size()));
