...
int BarelyMusician::CreateSequencer() {
  ...
  task_runner_.Add([this, sequencer_id]() {
    Sequencer sequencer(sample_rate_);
    sequencers_.insert(std::make_pair(sequencer_id, std::move(sequencer)));
  });
  return sequencer_id;
}
...
void BarelyMusician::ProcessSequencer(int sequencer_id) {
  Sequencer* sequencer = GetSequencer(sequencer_id);
  if (sequencer != nullptr) {
    sequencer->Update(num_frames_);
  }
}
...
void BarelyMusician::Update() { task_runner_.Run(); }
