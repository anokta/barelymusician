...
bool BarelyEngine_Create(int32_t sample_rate, int32_t max_sample_count, float reference_frequency,
                         BarelyEngineHandle* out_engine) {
  if (sample_rate <= 0) return false;
  if (max_sample_count <= 0) return false;
  if (reference_frequency <= 0.0) return false;
  if (!out_engine) return false;

  *out_engine = new BarelyEngine(sample_rate, max_sample_count, reference_frequency);
  return true;
}
...
bool BarelyEngine_Process(BarelyEngineHandle engine, float* output_samples,
  ...
  if (output_sample_count <= 0 || output_sample_count % 2 == 1) return false;
