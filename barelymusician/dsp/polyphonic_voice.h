...
class PolyphonicVoice {
 public:
  ...
  /// Starts new voice for a given note.
  ///
  /// @param note_id Note identifier.
  /// @param init_voice Callback to initialize the voice for playback.
  void Start(int note_id, const VoiceCallback& init_voice = nullptr) noexcept;

  /// Stops the voice with a given note.
  ///
  /// @param note_id Note identifier.
  /// @param shutdown_voice Callback to shutdown the voice.
  void Stop(int note_id, const VoiceCallback& shutdown_voice = nullptr) noexcept;
 ...
 private:
  ...
  // List of voice states, namely the voice note identifier and its timestamp. Timestamp is used to
  // determine which voice to *steal* when there is no free voice available.
  // TODO(#12): Consider a more optimized implementation for voice stealing.
  std::vector<std::pair<int, int>> voice_states_;
};
...
template <class VoiceType>
void PolyphonicVoice<VoiceType>::Start(int note_id, const VoiceCallback& init_voice) noexcept {
  ...
  voice_states_[voice_index] = {note_id, 0};
...
template <class VoiceType>
void PolyphonicVoice<VoiceType>::Stop(int note_id, const VoiceCallback& shutdown_voice) noexcept {
  const int voice_count = static_cast<int>(voices_.size());
  for (int i = 0; i < voice_count; ++i) {
    if (voice_states_[i].first == note_id && voices_[i].IsActive()) {
