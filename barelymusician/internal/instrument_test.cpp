...
// Tests that the instrument converts between frames and seconds as expected.
TEST(InstrumentTest, FramesSecondsConversion) {
  ...
  for (int i = 0; i < kValueCount; ++i) {
    EXPECT_EQ(instrument.GetFramesFromSeconds(kSeconds[i]), kFrames[i]);
    EXPECT_DOUBLE_EQ(instrument.GetSecondsFromFrames(kFrames[i]), kSeconds[i]);

    // Verify that the back and forth conversions do not mutate the value.
    EXPECT_EQ(instrument.GetFramesFromSeconds(instrument.GetSecondsFromFrames(kFrames[i])),
              kFrames[i]);
    EXPECT_DOUBLE_EQ(instrument.GetSecondsFromFrames(instrument.GetFramesFromSeconds(kSeconds[i])),
                     kSeconds[i]);
  }
}
