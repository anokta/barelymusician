...
bool BarelyEngine_Process(BarelyEngineHandle engine, float* output_samples,
  ...
  engine->Process({output_samples, output_samples + output_sample_count});
  return true;
}
...
bool BarelyEngine_Update(BarelyEngineHandle engine, double lookahead) {
  if (!engine) return false;

  engine->Update(lookahead);
  return true;
}
