  ...
  if (type == BarelyInstrumentControlType_kVoiceCount) {
    ...
    while (active_voice_index != UINT32_MAX) {
      ...
      if (voice.previous_voice_index != UINT32_MAX) {
        engine_.voice_pool.Get(voice.previous_voice_index).next_voice_index = UINT32_MAX;
...
void InstrumentProcessor::SetNoteOn(
  ...
  auto& params = engine_.instrument_params[instrument_index];
  if (const uint32_t voice_index = AcquireVoice(instrument_index, pitch);
      voice_index != UINT32_MAX) {
