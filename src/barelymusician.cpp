...
float BarelyLfo_Evaluate(BarelyEngine* engine, uint32_t lfo_id) {
  return (engine != nullptr && engine->IsValidLfo(lfo_id))
             ? engine->controller.lfo_controller().Evaluate(engine->state.GetIdIndex(lfo_id))
             : 0.0f;
}

double BarelyLfo_GetPhase(const BarelyEngine* engine, uint32_t lfo_id) {
  return (engine != nullptr && engine->IsValidLfo(lfo_id))
             ? engine->state.GetLfo(engine->state.GetIdIndex(lfo_id)).phase
             : 0.0;
}
...
void BarelyLfo_SetPhase(BarelyEngine* engine, uint32_t lfo_id, double phase) {
  if (engine != nullptr && engine->IsValidLfo(lfo_id)) {
    engine->controller.lfo_controller().SetPhase(engine->state.GetIdIndex(lfo_id), phase);
  }
}
