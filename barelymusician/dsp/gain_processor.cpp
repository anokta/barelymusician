...
GainProcessor::GainProcessor(int frame_rate) noexcept
    : unity_ramp_frame_count_(static_cast<double>(frame_rate) * kUnityRampDurationSeconds) {}
...
void GainProcessor::Process(double* buffer, int channel_count, int frame_count) noexcept {
  ...
  if (gain_ != target_gain_) {
    if (is_initialized_) {
      frame = static_cast<int>(unity_ramp_frame_count_ * std::abs(target_gain_ - gain_));
      if (frame > 0) {
        gain_ = ApplyLinearRamp(gain_, target_gain_, frame, buffer, channel_count, frame_count);
      }
    } else {
      gain_ = target_gain_;
    }
  ...
  if (frame < frame_count) {
    ApplyConstantGain(gain_, &buffer[channel_count * frame], channel_count, frame_count - frame);
  }
}
