...
int main(int /*argc*/, char* /*argv*/[]) {
  ...
  performer.CreateRecurringTask(play_note_fn(0, 1.0), 0.0);
  // Trigger 2.
  triggers.emplace_back(1.0, 1.0);
  performer.CreateRecurringTask(play_note_fn(1, 1.0), 1.0);
  // Trigger 3.
  triggers.emplace_back(2.0, 1.0);
  performer.CreateRecurringTask(play_note_fn(2, 1.0), 2.0);
  // Trigger 4.
  triggers.emplace_back(3.0, 1.0);
  performer.CreateRecurringTask(play_note_fn(3, 0.66), 3.0);
  performer.CreateRecurringTask(play_note_fn(4, 0.34), 3.66);
  // Trigger 5.
  triggers.emplace_back(4.0, 1.0);
  performer.CreateRecurringTask(play_note_fn(5, 0.33), 4.0);
  performer.CreateRecurringTask(play_note_fn(6, 0.33), 4.33);
  performer.CreateRecurringTask(play_note_fn(7, 0.34), 4.66);
  // Trigger 6.
  triggers.emplace_back(5.0, 2.0);
  performer.CreateRecurringTask(play_note_fn(8, 2.0), 5.0);

  // Stopper.
  auto stopper = performer.CreateRecurringTask(
      [&performer]() { performer.Stop(); }, 0.0, -1);
