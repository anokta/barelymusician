...
void InstrumentController::SetAllNotesOff(uint32_t instrument_index) noexcept {
  ...
  if (instrument.arp.is_active) {
    ...
    engine_.ScheduleMessage(NoteOffMessage{instrument_index, instrument.arp.pitch});
  } else {
...
void InstrumentController::ProcessArp(MainRng& main_rng) noexcept {
  for (uint32_t i = 0; i < engine_.instrument_pool.GetActiveCount(); ++i) {
    ...
    if (!instrument.arp.is_active && instrument.arp.phase == 0.0) {
      ...
      engine_.ScheduleMessage(
          NoteOnMessage{engine_.instrument_pool.GetActiveIndex(i), instrument.arp.pitch,
      ...
      instrument.note_event_callback(BarelyNoteEventType_kEnd, instrument.arp.pitch);
      engine_.ScheduleMessage(
          NoteOffMessage{engine_.instrument_pool.GetActiveIndex(i), instrument.arp.pitch});
    }
  }
}
